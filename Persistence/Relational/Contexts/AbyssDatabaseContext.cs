...
{
    public static async Task<TRow> GetOrCreateRowAsync<TRow>(this AbyssDatabaseContext context, object primaryKey) where TRow : class, new()
    {
    ...
    {
        var context = dbSet.GetService<AbyssDatabaseContext>();
...
public class AbyssDatabaseContext: DbContext
{
    ...
    public AbyssDatabaseContext(DbContextOptions<AbyssDatabaseContext> options, IConfiguration config, IServiceProvider provider) : base(options)
    {
    ...
    {
        modelBuilder.Entity<DocumentRecord<ulong, GuildConfig>>().ToTable("guilds");
