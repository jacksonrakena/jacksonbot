...
public class EvaluationHelper
{
    ...
    public CachedUser? User(ulong id)
    {
        return Context.Services.GetRequiredService<DiscordBot>().GetUser(id);
    }

    public CachedUser? User(string username)
    {
        if (!Context.Services.GetRequiredService<DiscordBot>().CacheProvider.TryGetUsers(out var users)) return null;
        return users.Values.FirstOrDefault(a =>
