...
namespace Abyss.Core.Checks.Parameter
{
    ...
    public class MustNotBeInvokerAttribute : ParameterCheckAttribute, IAbyssCheck
    {
        ...
        public string GetDescription(AbyssRequestContext requestContext)
        {
            return "The provided user can't be you.";
        }
            ...
            IServiceProvider provider)
        {
            ...
            var id = argument is SocketUser user ? user.Id :
                argument is DiscordUserReference dur ? dur.Id :
                throw new InvalidOperationException(
                    $"{nameof(MustNotBeInvokerAttribute)} is being executed on an invalid object type. Expected a SocketUser or DiscordUserReference variant, got {argument.GetType().Name}.");
            ;
