...
namespace Rosalina
{
    public class RosalinaBot : DiscordBot
    {
        ...
        public async Task HandleRuntimeExceptionAsync(RosalinaCommandContext context, Exception exception, CommandExecutionStep step, string reason)
        {
        ...
        public async Task HandleCommandExecutedAsync(CommandExecutedEventArgs args)
        {
            ...
            var context = ctx.AsRosalinaContext();
            var logger = context.Logger; 

            if (!(result is RosalinaResult baseResult))
            {
                if (result == null)
                {
                    logger.Error("Command {Name} returned a null result type.", command.Name);
                } else logger.Error("Command {Name} returned a result of type {TypeName} and not {ResultTypeName}.", command.Name, result.GetType().Name, typeof(RosalinaResult).Name);
                await context.Channel.TrySendMessageAsync($"Man, this bot sucks. Command {command.Name} is broken, and will need to be recompiled. Try again later. (Developer: The command returned a type that isn't a {typeof(RosalinaResult).Name}.)");
                return;
            }
        ...
        {
            return HandleRuntimeExceptionAsync(e.Context.AsRosalinaContext(), e.Result.Exception, e.Result.CommandExecutionStep, e.Result.Reason);
        }

        protected override async ValueTask AfterExecutedAsync(IResult result, DiscordCommandContext rawContext)
        {
            var context = rawContext.AsRosalinaContext();
