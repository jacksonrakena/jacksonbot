...
namespace Rosalina
{
    ...
    public class HelpModule : RosalinaModuleBase
    {
        private readonly HelpService _help;
        private readonly RosalinaBot _bot;

        public HelpModule(HelpService help, RosalinaBot bot)
        {
            _bot = bot;
            _help = help;
        }

        public async Task<RosalinaResult> CommandSubroutine_HelpQueryAsync(string query)
        {
        ...
        public async Task<RosalinaResult> Command_ListCommandsAsync(
            [Name("Query")]
